...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if(p(elem)) new NonEmpty(elem, new Empty, new Empty)
    else new Empty
  }

  def filter(p: Tweet => Boolean): TweetSet = {
    filterAcc(p, this) union (left.filter(p)) union (right.filter(p))
  }
...
//      else {
        val max = if (!left.isEmpty) {
          val mostLeft = left.mostRetweeted

          if (elem.retweets > mostLeft.retweets)
            elem
          else mostLeft
        ...
        if (!right.isEmpty) {
          ...
          if (max.retweets > mostRight.retweets)
            max
          else mostRight
...
object GoogleVsApple {
  ...
  println("Finished reading tweets")
